...
def wasserstein_loss(pred, target, eps=1e-7, constant=12.8):
    ...
    # epsilon = 1e-8
    # term1 = w1 ** 2 / (h1 ** 2 + w1 ** 2 + epsilon)
    # term2 = w2 ** 2 / (h2 ** 2 + w2 ** 2 + epsilon)
    # term1 = (w1 / h1)**2
    # term2 = (w2 /h2)**2
    # diff = term1 - term2
    # # 使用clamp确保差值非负，并添加epsilon保证数值稳定性
    # v = torch.sqrt(torch.clamp(diff, min=0) + epsilon)
    # v = torch.clamp(diff, min=0)
    v=(w1/h1-w2/h2)**2
    # v = ((w1 / h1 - w2 / h2) ** 2)/(w2*h2)
    # Total Wasserstein distance
    wasserstein_2 = center_distance + wh_distance+0.5*v
...
class Metric(SimpleClass):
    ...
    def fitness(self):
        ...
        w = [0.0, 0.0, 0.1,0.0, 0.9]  # weights for [P, R, mAP@0.5, mAP@0.5:0.95]0.1, 0.9]
