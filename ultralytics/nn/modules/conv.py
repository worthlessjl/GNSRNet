...
class SADN(nn.Module):
    ...
    def forward(self, x):
        ...
        query = self.query_conv(x).view(batch_size, -1, H * W).permute(0, 2, 1) # B x N x C'
        key = self.key_conv(x).view(batch_size, -1, H * W) # B x C' x N
        ...
        out = out.view(batch_size, C, H, W) # 重塑为原始尺寸

        out = x - self.gamma * out  # 残差连接
        return out
